...
import { ACTION, ACTION_SUBJECT, EVENT_TYPE, FULL_WIDTH_MODE, getAnalyticsEventsFromTransaction, PLATFORMS } from '../plugins/analytics';
...
import { PROSEMIRROR_RENDERED_NORMAL_SEVERITY_THRESHOLD, PROSEMIRROR_RENDERED_DEGRADED_SEVERITY_THRESHOLD } from './consts';
...
export default class ReactEditorView extends React.Component {
  ...
  get transactionTrackingProp() {
    ...
    const {
      transactionTracking
    } = editorProps.performanceTracking ? editorProps.performanceTracking : editorProps;
  ...
  constructor(_props, context) {
    ...
    _defineProperty(this, "pluginPerformanceObserver", new PluginPerformanceObserver(report => this.onPluginObservation(report, this.editorState)).withPlugins(() => this.getPluginNames()).withNodeCounts(() => this.countNodes()).withOptions(() => this.transactionTrackingOptions));
    ...
    _defineProperty(this, "reconfigureState", props => {
      ...
      this.config = processPluginsList(this.getPlugins(props.editorProps, this.props.editorProps, props.createAnalyticsEvent));
    ...
    _defineProperty(this, "createEditorState", options => {
      ...
      if (options.replaceDoc) {
        doc = processRawValue(schema, defaultValue, options.props.providerFactory, options.props.editorProps.sanitizePrivateContent, this.contentTransformer, this.dispatchAnalyticsEvent);
      }
    ...
    _defineProperty(this, "dispatchTransaction", transaction => {
      ...
      shouldTrack && stopMeasure(`🦉 ReactEditorView::dispatchTransaction`, () => {});
    });
    ...
    this.errorReporter = createErrorReporter(_props.editorProps.errorReporterHandler); // This needs to be before initialising editorState because
  ...
  UNSAFE_componentWillReceiveProps(nextProps) {
    ...
    if (this.props.allowAnalyticsGASV3 !== nextProps.allowAnalyticsGASV3) {
      ...
      if (this.props.allowAnalyticsGASV3 && nextProps.createAnalyticsEvent !== this.props.createAnalyticsEvent) {
