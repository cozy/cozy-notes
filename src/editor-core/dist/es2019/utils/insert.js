...
const isLastChild = ($pos, doc) =>
  doc.resolve($pos.after()).node().lastChild === $pos.node()

const isFirstChild = ($pos, doc) =>
...
export const safeInsert = (content, position) => tr => {
  const { nodes } = tr.doc.type.schema
  const whitelist = [nodes.rule, nodes.mediaSingle] // fallback if the node to insert is not in the whitelist, or if the insertion should happen within a list.
  ...
  const insertPosStart = $insertPos.start() // When parent node is an empty paragraph,
  ...
  while ($proposedPosition.depth > 0) {
    ...
    const parentNode = $parentPos.node() // Insert at position (before or after target pos)
...
export const insertSelectedItem = (maybeNode, opts = {}) => (
  state,
  tr,
  start
) => {
