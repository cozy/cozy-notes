...
export function defaultInputRuleHandler(inputRule, isBlockNodeRule = false) {
  ...
  inputRule.handler = (state, match, start, end) => {
    ...
    if ($from.parent.type.spec.code || !(state.selection instanceof TextSelection) || unsupportedMarks) {
      return;
    }
...
export function instrumentedInputRule(pluginName, {
  rules
}) {
...
const hasUnsupportedMarkForBlockInputRule = (state, start, end) => {
  ...
  const isUnsupportedMark = node => node.type === marks.code || node.type === marks.link || node.type === marks.typeAheadQuery;
