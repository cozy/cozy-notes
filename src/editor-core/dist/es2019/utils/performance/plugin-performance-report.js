...
export class PluginPerformanceReport {
  ...
  withEntryList(entryList) {
    ...
    this.stateApplied = this.getEntryByName(entryList, '🦉 EditorView::state::apply');
    this.viewUpdated = this.getEntryByName(entryList, '🦉 EditorView::updateState');
    this.onChangeCalled = this.getEntryByName(entryList, '🦉 ReactEditorView::onChange');
    this.onEditorViewStateUpdatedCalled = this.getEntryByName(entryList, '🦉 ReactEditorView::onEditorViewStateUpdated');
  ...
  withPlugins(pluginNames) {
    ...
    if (this.stateApplied && pluginNames.length > 0) {
      ...
      this.slowPlugins = pluginEntries.filter(pluginIsOutlier).reduce((acc, [n, d]) => ({ ...acc,
        [n]: d
      }), {});
    }
