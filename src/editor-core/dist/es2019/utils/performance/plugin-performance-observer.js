...
export class PluginPerformanceObserver {
  constructor(callback) {
    ...
    _defineProperty(this, "onObserveration", entries => {
      const reports = entries.getEntriesByName('🦉 ReactEditorView::dispatchTransaction').map(entry => PluginPerformanceReport.fromEntry(entry).withCount(++this.reportCount).withEntryList(entries).withNodes(...this.getNodeCounts()).withPlugins(this.getPlugins()).withOptions(this.getOptions()).toJSON());
      reports.filter(report => report.trigger !== 'none').forEach(report => this.callback(report));
    });

    _defineProperty(this, "observer", window.PerformanceObserver ? new PerformanceObserver(this.onObserveration) : {
      observe() {},
