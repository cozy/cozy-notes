...
export function shouldRecalcDecorations(pluginState, state) {
  const { selection: oldSelection, decorationSet } = pluginState
  const { selection: newSelection } = state // If selection is unchanged, no need to recalculate
  ...
  if (oldSelection.eq(newSelection)) {
    ...
    if (
      ...
      const newDecorations = getDecorations(state.tr).find() // There might not be old or new decorations if the node selection is for a text node
...
export const isSelectableContainerNode = node =>
  !!(node && !node.isAtom && NodeSelection.isSelectable(node))
export const isSelectableChildNode = node =>
  !!(
    node &&
...
export const findFirstChildNodeToSelect = parent =>
  flatten(parent).find(
    child =>
