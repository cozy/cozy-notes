...
export default function keymapPlugin() {
  const map = {};
  keymaps.bindKeymapWithCommand(keymaps.insertNewLine.common, (state, dispatch, view) => {
    const isInGapCursor = state.selection instanceof GapCursorSelection; // Only operate in gap cursor
  ...
  keymaps.bindKeymapWithCommand(keymaps.moveLeft.common, (state, dispatch, view) => {
  ...
  keymaps.bindKeymapWithCommand(keymaps.moveRight.common, (state, dispatch, view) => {
    const endOfTextblock = view ? view.endOfTextblock.bind(view) : undefined;
    return arrow(Direction.RIGHT, endOfTextblock)(state, dispatch);
  }, map);
  keymaps.bindKeymapWithCommand(keymaps.moveUp.common, (state, dispatch, view) => {
    const endOfTextblock = view ? view.endOfTextblock.bind(view) : undefined;
    return arrow(Direction.UP, endOfTextblock)(state, dispatch);
  }, map);
  keymaps.bindKeymapWithCommand(keymaps.moveDown.common, (state, dispatch, view) => {
    const endOfTextblock = view ? view.endOfTextblock.bind(view) : undefined;
