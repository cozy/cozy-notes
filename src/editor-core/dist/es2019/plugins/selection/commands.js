...
import { getDecorations, isSelectableContainerNode, isSelectionAtEndOfParentNode, findSelectableContainerParent, isSelectionAtStartOfParentNode, findSelectableContainerBefore, findSelectableContainerAfter, findFirstChildNodeToSelect, findLastChildNodeToSelect } from './utils';
import { RelativeSelectionPos, SelectionDirection } from './types';
export const setDecorations = () => createCommand(state => ({
...
const setSelectionInsideAtNodeEnd = (selectionRelativeToNode, node, from, to) => (state, dispatch) => {
  if (isNodeEmpty(node)) {
