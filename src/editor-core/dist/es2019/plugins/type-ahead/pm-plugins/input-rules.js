...
import { pluginKey as typeAheadPluginKey } from './main'; // We cannot set a proper plugin key on input rule plugins, so instead, once
...
export function inputRulePlugin(schema, typeAheads) {
  const triggersRegex = typeAheads.map(t => t.customRegex || t.trigger).join('|');

  if (!triggersRegex.length) {
    return;
  }

  const regex = new RegExp(`(^|[.!?\\s${leafNodeReplacementCharacter}])(${triggersRegex})$`);
  const typeAheadInputRule = createInputRule(regex, (state, match) => {
    const typeAheadState = typeAheadPluginKey.getState(state);
    /**
    ...
    const mark = schema.mark('typeAheadQuery', {
      trigger
    });
    const {
      tr,
