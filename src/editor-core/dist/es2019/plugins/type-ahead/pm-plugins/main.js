      ...
      apply(tr, pluginState, _oldState, state) {
        ...
        switch (action) {
          ...
          case ACTIONS.SELECT_NEXT:
            return selectNextActionHandler({
              dispatch,
              pluginState,
              tr
            })
          ...
          case ACTIONS.ITEMS_LIST_UPDATED:
            return itemsListUpdatedActionHandler({
              dispatch,
              pluginState,
              tr
            })
          ...
          case ACTIONS.SELECT_CURRENT:
            ...
            const { typeAheadQuery } = state.schema.marks // If inserted content has typeAheadQuery mark should fallback to default action handler
    ...
    view() {
      return {
        update(editorView) {
          ...
          const { typeAheadQuery } = state.schema.marks // Disable type ahead query when removing trigger.
...
export function setCurrentItemIndex({ dispatch, pluginState, params }) {
  if (!params) {
    return pluginState
  }
...
export function updateQueryHandler({ dispatch, pluginState, params }) {
  if (!params) {
    return pluginState
  }
...
export function selectPrevActionHandler({ dispatch, pluginState }) {
...
export function selectNextActionHandler({ dispatch, pluginState }) {
  const newIndex = pluginState.currentIndex + 1
  const newPluginState = {
    ...pluginState,
...
export function itemsListUpdatedActionHandler({ dispatch, pluginState, tr }) {
...
export function selectCurrentActionHandler({ dispatch }) {
