...
export function createInitialPluginState(prevActiveState = false, isAllowed = true) {
  return {
    isAllowed,
    active: false,
    prevActiveState,
...
export function createPlugin(dispatch, reactContext, typeAhead) {
  return new Plugin({
      ...
      apply(tr, pluginState, _oldState, state) {
        ...
        switch (action) {
          ...
          case ACTIONS.SELECT_CURRENT:
            ...
            return tr.doc.rangeHasMark(from - 1, to, typeAheadQuery) ? defaultActionHandler({
              dispatch,
              reactContext,
              typeAhead,
              state,
    ...
    appendTransaction(_trs, _oldState, newState) {
      ...
      if (pluginState.active && pluginState.query && pluginState.typeAheadHandler && pluginState.typeAheadHandler.forceSelect && pluginState.typeAheadHandler.forceSelect(pluginState.query, pluginState.items)) {
        let newTr;
    ...
    props: {
      handleDOMEvents: {
        ...
        compositionend: (view, event) => {
          ...
          if (triggers.indexOf(event.data) !== -1 && selection instanceof TextSelection && selection.$cursor && !schema.marks.typeAheadQuery.isInSet(selection.$cursor.marks())) {
