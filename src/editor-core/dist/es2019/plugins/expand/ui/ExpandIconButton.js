...
const ExpandLayoutWrapperWithRef = /*#__PURE__*/ forwardRef(function WithRef(
  props,
  ref
) {
...
export const ExpandIconButton = props => {
  ...
  const supportsAnyHover = window.matchMedia
    ? window.matchMedia('(any-hover: hover)').matches !==
      window.matchMedia('(any-hover: none)').matches
    : false
  const hoverEventCheck = supportsAnyHover
    ? window.matchMedia('(any-hover: hover)').matches
    : true // hoverEventCheck is to disable tooltips for mobile to prevent incorrect hover state causing issues on iOS
