...
const isExpandSelected = selection => selection instanceof NodeSelection && (selection.node.type.name === 'expand' || selection.node.type.name === 'nestedExpand');

export function expandKeymap() {
  const list = {};
  keymaps.bindKeymapWithCommand(keymaps.moveRight.common, (state, dispatch, editorView) => {
    if (!editorView) {
      return false;
    }
  ...
  keymaps.bindKeymapWithCommand(keymaps.moveLeft.common, (state, dispatch, editorView) => {
    if (!editorView) {
      return false;
    }
  ...
  keymaps.bindKeymapWithCommand(keymaps.tab.common, (state, dispatch, editorView) => {
  ...
  keymaps.bindKeymapWithCommand(keymaps.moveUp.common, (state, dispatch, editorView) => {
  ...
  keymaps.bindKeymapWithCommand(keymaps.moveDown.common, (state, dispatch, editorView) => {
    if (!editorView) {
      return false;
    }
  ...
  keymaps.bindKeymapWithCommand(keymaps.backspace.common, (state, dispatch, editorView) => {
    const {
      selection
    } = state;
