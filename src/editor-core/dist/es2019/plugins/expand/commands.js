...
export const deleteExpandAtPos = (expandNodePos, expandNode) => (
  state,
  dispatch
) => {
...
export const deleteExpand = () => (state, dispatch) => {
...
export const updateExpandTitle = (title, pos, nodeType) => (
  state,
  dispatch
) => {
  ...
  if (node && node.type === nodeType && dispatch) {
    const { tr } = state
    tr.setNodeMarkup(pos, node.type, { ...node.attrs, title }, node.marks)
    dispatch(tr)
  }
...
export const toggleExpandExpanded = (pos, nodeType) => (state, dispatch) => {
  ...
  if (node && node.type === nodeType && dispatch) {
    ...
    tr.setNodeMarkup(
      pos,
      node.type,
      { ...node.attrs, __expanded: isExpandedNext },
      node.marks
