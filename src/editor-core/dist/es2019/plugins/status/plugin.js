...
const createPlugin = (dispatch, portalProviderAPI, eventDispatcher, options) =>
  new Plugin({
    state: {
      init: () => ({
        isNew: false,
      ...
      apply(tr, state, oldEditorState) {
        ...
        if (tr.docChanged && state.showStatusPickerAt) {
          ...
          const newState = { ...state, showStatusPickerAt }
        ...
        if (!tr.selection.eq(oldEditorState.selection)) {
          ...
          if (showStatusPickerAt !== state.showStatusPickerAt) {
            const newState = { ...state, isNew: false, showStatusPickerAt }
            dispatch(pluginKey, newState)
            return newState
          }
        }
