...
const createPlugin = (dispatch, portalProviderAPI, eventDispatcher, options) => new Plugin({
  state: {
    init: () => ({
      isNew: false,
  ...
  filterTransaction: (tr, state) => {
    ...
    if (tr.selectionSet && !tr.steps.length && tr.isGeneric && tr.selection instanceof TextSelection && state.selection instanceof NodeSelection) {
      ...
      const nodeAtSelection = tr.doc.nodeAt(tr.selection.from); // prevent changing node selection to text selection on dom change right after inserting status
  ...
  appendTransaction: (transactions, oldEditorState, newEditorState) => {
    let changed = false;
    let tr = newEditorState.tr; // user leaves the StatusPicker with empty text and selects a new node
  ...
  props: {
    ...
    decorations(state) {
      ...
      if (options && options.allowZeroWidthSpaceAfter && nodeAtSelection && nodeAtSelection.type === state.schema.nodes.status) {
        const delayedNodeRendering = () => {
          return document.createTextNode(ZERO_WIDTH_SPACE);
        };

        const decoration = Decoration.widget(tr.selection.from, delayedNodeRendering, {
          side: 1,
