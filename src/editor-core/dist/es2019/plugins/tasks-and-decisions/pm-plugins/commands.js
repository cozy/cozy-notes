...
export const joinAtCut = $pos => (state, dispatch) => {
  ...
  const { paragraph } = $cut.doc.type.schema.nodes // find the boundary between the taskList and paragraph
  ...
  if (
    ...
    let $lastNode = $cut.doc.resolve($cut.pos - 1) // might have deeply nested taskList, keep trying to find it
    ...
    const slice = state.tr.doc.slice($lastNode.pos, $cut.pos) // collapse the range between end of last taskItem and after the paragraph
