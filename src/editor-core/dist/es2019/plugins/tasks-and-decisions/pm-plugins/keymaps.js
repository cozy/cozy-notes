...
const deleteHandler = filter(
  ...
  chainCommands(joinTaskDecisionFollowing, (state, dispatch) => {
    ...
    const { taskList, paragraph, doc } = state.schema.nodes // this is a top-level node it wont have $next.before()
    ...
    if (
      $next.node($next.depth - 2).type === taskList || // this is for the case when we are on a non-nested item and next one is nested
