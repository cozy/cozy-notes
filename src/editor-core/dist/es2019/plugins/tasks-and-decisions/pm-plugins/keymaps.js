...
import { getBlockRange, getCurrentIndentLevel, isActionOrDecisionItem, isActionOrDecisionList, isEmptyTaskDecision, isInsideTask, isInsideTaskOrDecisionItem, liftBlock, subtreeHeight, walkOut, isTable } from './helpers';
...
const joinTaskDecisionFollowing = (state, dispatch) => {
  ...
  const $next = walkOut(state.selection.$from); // if there's no taskItem or taskList following, then
  ...
  if (!parentList) {
    ...
    if ($next.parent.type === bulletList || $next.parent.type === orderedList) {
      ...
      if ($next.parent.firstChild && $next.parent.firstChild.type === listItem) {
        ...
        const resolvedStartPos = state.doc.resolve($next.pos + 1); // Unindent the first listItem.
        // As if placing your cursor just after the first dot of the list (before the text)
        // and pressing Shift-Tab.

        const tr = liftBlock(state.tr, resolvedStartPos, resolvedStartPos); // If autoJoin not used, two ul/ol elements appear rather than one with multiple li elements
...
const backspaceFrom = $from => (state, dispatch) => {
  ...
  if (actionDecisionFollowsOrNothing($from)) {
    if (dispatch) {
      const taskContent = state.doc.slice($from.start(), $from.end()).content; // might be end of document after
...
const deleteHandler = filter([isInsideTaskOrDecisionItem, isEmptySelectionAtEnd], chainCommands(joinTaskDecisionFollowing, (state, dispatch) => {
  // look for the node after this current one
  ...
  const {
    ...
    paragraph,
    doc
  } = state.schema.nodes; // this is a top-level node it wont have $next.before()
  ...
  if (actionDecisionFollowsOrNothing(state.selection.$from)) {
    if (dispatch) {
      const taskContent = state.doc.slice($next.start(), $next.end()).content; // might be end of document after
...
const splitListItemWith = (tr, content, $from, setSelection) => {
  const origDoc = tr.doc; // split just before the current item
  ...
  const $oldAfter = origDoc.resolve($from.after()); // if different levels then we shouldn't lift
...
export function keymapPlugin(schema, allowNestedTasks, consumeTabs) {
  const indentHandlers = {
    'Shift-Tab': consumeTabs ? chainCommands(unindent, isInsideTaskOrDecisionItem) : unindent,
    Tab: consumeTabs ? chainCommands(indent, isInsideTaskOrDecisionItem) : indent
  };
