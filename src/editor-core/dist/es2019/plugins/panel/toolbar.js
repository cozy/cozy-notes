...
export const getToolbarItems = (formatMessage, panelNodeType, isCustomPanelEnabled, providerFactory, activePanelType, activePanelColor, activePanelIcon) => {
  ...
  if (isCustomPanelEnabled) {
    items.push({
      type: 'custom',
      render: (view, idx) => /*#__PURE__*/React.createElement(EmojiPickerButton, {
        key: idx,
        view: view,
        title: formatMessage(messages.emoji),
        providerFactory: providerFactory,
        isSelected: activePanelType === PanelType.CUSTOM && !!activePanelIcon,
        onChange: emoji => {
          if (!view) {
            return;
          }
          ...
          }, isCustomPanelEnabled)(view.state, view.dispatch);
        }
      })
    }, {
      type: 'separator'
...
export const getToolbarConfig = (state, intl, options = {}, providerFactory) => {
  ...
  if (panelState && panelState.toolbarVisible && panelState.element) {
    ...
    const items = getToolbarItems(formatMessage, nodeType, options.UNSAFE_allowCustomPanel || false, providerFactory, activePanelType, activePanelColor, activePanelIcon);
    return {
