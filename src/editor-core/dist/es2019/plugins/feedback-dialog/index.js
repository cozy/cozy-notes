...
export const openFeedbackDialog = async (feedbackInfo) => new Promise(async (resolve, reject) => {
  const combinedFeedbackInfo = { ...defaultFeedbackInfo,
    ...feedbackInfo
  };
  ...
  if (!showJiraCollectorDialog || feedbackInfoHash !== newFeedbackInfoHash) {
    try {
      showJiraCollectorDialog = await loadJiraCollectorDialogScript([combinedFeedbackInfo.product || 'n/a', ...(combinedFeedbackInfo.labels || [])], combinedFeedbackInfo.packageName || '', coreVersion, combinedFeedbackInfo.packageVersion || '');
  ...
  return {
    ...
    pluginsOptions: {
      quickInsert: ({
        ...
        priority: 400,
        keywords: ['bug'],
        icon: () => /*#__PURE__*/React.createElement(IconFeedback, {
          label: formatMessage(messages.feedbackDialog)
        }),
