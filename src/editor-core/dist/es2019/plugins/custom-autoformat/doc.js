...
export const buildHandler = (_regex, handler) => {
  return async (view, match, start, end) => {
    const replacementPromise = handler(match.slice(1, match.length - 1)); // queue the position and match pair so that we can remap across transactions
...
export const completeReplacements = (view, state) => {
  ...
  state.matches.forEach(completedMatch => {
    ...
    matchingRequests.forEach(request => {
      ...
      const matchEndPos = end + suffix.length; // only permit inlineCard as replacement target for now
      ...
      const docText = view.state.doc.textBetween(start, matchEndPos, '#', '|'); // only replace if text still remains the same as when typed at the start
