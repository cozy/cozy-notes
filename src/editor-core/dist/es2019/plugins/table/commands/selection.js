...
const selectFullTable = ({ node, startPos, dir }) => (state, dispatch) => {
  const { doc } = state
...
const setSelectionAtStartOfFirstCell = (selection, selectionRelativeToNode) => (
  state,
  dispatch
) => {
...
const setSelectionAtEndOfLastCell = (selection, selectionRelativeToNode) => (
  state,
  dispatch
) => {
  const { $anchorCell, $headCell } = selection
