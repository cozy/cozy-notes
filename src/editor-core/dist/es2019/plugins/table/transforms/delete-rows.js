...
export const deleteRows = (rect, isHeaderRowRequired = false) => tr => {
        ...
        if (cellsInRow.indexOf(cellPos) > -1) {
          ...
          if (overlappingRows > 0) {
            const newCell = cell.type.createChecked(
              { ...cell.attrs, rowspan: cell.attrs.rowspan - overlappingRows },
              cell.content,
              cell.marks
            )
          ...
          const attrs = { ...cell.attrs, colspan: 1, rowspan: 1 }
  ...
  return setMeta({
    ...
    tr
      .replaceWith(table.pos, table.pos + table.node.nodeSize, fixedTable) // move cursor before the deleted rows if possible, otherwise - to the first row
