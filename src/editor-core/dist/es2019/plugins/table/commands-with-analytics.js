...
import { ACTION_SUBJECT, EVENT_TYPE, INPUT_METHOD, TABLE_ACTION, TABLE_BREAKOUT, withAnalytics } from '../analytics';
...
export const emptyMultipleCellsWithAnalytics = (inputMethod, targetCellPosition) => withAnalytics(({
  selection
}) => {
  const {
    horizontalCells,
...
export const mergeCellsWithAnalytics = () => withAnalytics(({
  selection
}) => {
  const {
    horizontalCells,
...
export const splitCellWithAnalytics = () => withAnalytics(({
  selection
}) => {
  const {
    totalRowCount,
...
export const setColorWithAnalytics = (cellColor, targetCellPosition) => withAnalytics(({
  selection
}) => {
  const {
    horizontalCells,
...
export const addRowAroundSelection = side => (state, dispatch) => {
  const {
    selection
  } = state;
...
export const insertRowWithAnalytics = (inputMethod, options) => withAnalytics(state => {
  const {
    totalRowCount,
    totalColumnCount
...
export const insertColumnWithAnalytics = (inputMethod, position) => withAnalytics(state => {
  const {
    totalRowCount,
    totalColumnCount
  } = getSelectedTableInfo(state.selection);
...
export const deleteRowsWithAnalytics = (inputMethod, rect, isHeaderRowRequired) => withAnalytics(({
  selection
}) => {
  const {
    totalRowCount,
...
export const deleteColumnsWithAnalytics = (inputMethod, rect) => withAnalytics(({
  selection
}) => {
  const {
    totalRowCount,
...
export const deleteTableWithAnalytics = () => withAnalytics(({
  selection
}) => getTableDeletedAnalytics(selection, INPUT_METHOD.FLOATING_TB))(deleteTable);
export const deleteTableIfSelectedWithAnalytics = inputMethod => withAnalytics(({
  selection
}) => getTableDeletedAnalytics(selection, inputMethod))(deleteTableIfSelected);
export const toggleHeaderRowWithAnalytics = () => withAnalytics(state => {
  const {
    totalRowCount,
    totalColumnCount
  } = getSelectedTableInfo(state.selection);
...
export const toggleHeaderColumnWithAnalytics = () => withAnalytics(state => {
  const {
    totalRowCount,
    totalColumnCount
  } = getSelectedTableInfo(state.selection);
...
export const toggleNumberColumnWithAnalytics = () => withAnalytics(state => {
  const {
    totalRowCount,
    totalColumnCount
  } = getSelectedTableInfo(state.selection);
...
export const toggleTableLayoutWithAnalytics = () => withAnalytics(state => {
  const {
    table,
    totalRowCount,
    totalColumnCount
...
export const sortColumnWithAnalytics = (inputMethod, columnIndex, sortOrder) => withAnalytics(state => {
  const {
    totalRowCount,
    totalColumnCount
  } = getSelectedTableInfo(state.selection);
...
})(sortByColumn(columnIndex, sortOrder)); // #endregion