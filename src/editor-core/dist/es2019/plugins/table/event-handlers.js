...
import { isElementInTableCell, isLastItemMediaGroup, setNodeSelection } from '../../utils';
...
export const handleMouseMove = (view, event) => {
  ...
  if (isColumnControlsDecorations(element)) {
    ...
    const positionColumn = getMousePositionHorizontalRelativeByElement(event) === 'right' ? endIndex : startIndex;
  ...
  if (!isResizeHandleDecoration(element) && isCell(element)) {
    const positionColumn = getMousePositionHorizontalRelativeByElement(event, RESIZE_HANDLE_AREA_DECORATION_GAP);
...
export const handleCut = (oldTr, oldState, newState) => {
