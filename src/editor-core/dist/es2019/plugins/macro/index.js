...
export const createPlugin = (dispatch, providerFactory) => new Plugin({
  state: {
    init: () => ({
      macroProvider: null
    }),
  ...
  key: pluginKey,
  view: view => {
    const handleProvider = (_name, provider) => provider && setMacroProvider(provider)(view); // make sure editable DOM node is mounted
