...
import { applyTextMarksToSlice, hasOnlyNodesOfType, isEmptyNode, isCursorSelectionAtTextStartOrEnd, isSelectionInsidePanel } from './util';
import { getFeatureFlags } from '../feature-flags-context';
import { isListNode } from '../lists-predictable/utils/node'; // remove text attribute from mention for copy/paste (GDPR)
...
export function handlePasteAsPlainText(slice, _event) {
  return (state, dispatch, view) => {
    ...
    if (view && view.shiftKey) {
      ...
      const {
        selection
      } = tr; // <- using the same internal flag that prosemirror-view is using
      // if user has selected table we need custom logic to replace the table

      tr = replaceSelectedTable(state, slice, INPUT_METHOD.CLIPBOARD); // otherwise just replace the selection
...
export function handlePastePreservingMarks(slice) {
  return (state, dispatch) => {
    ...
    const textFormattingState = textFormattingPluginKey.getState(state); // special case for codeMark: will preserve mark only if codeMark is currently active
    ...
    const isPlainTextSlice = slice.content.childCount === 1 && slice.content.firstChild.type === paragraph && slice.content.firstChild.content.childCount === 1 && slice.content.firstChild.firstChild.type === text; // special case for plainTextSlice & linkMark: merge into existing link
...
export function insertSlice({
  ...
  const selectionIsInsideList = $from.blockRange($to, isListNode); // if pasting a list inside another list, ensure no empty list items get added
  ...
  if (panelNode && isEmptyNode(panelNode) && $from.node() === $to.node()) {
    const {
      from: panelPosition
    } = selection; // if content of slice isn't valid for a panel node, insert slice after
    ...
    if (panelNode && !panelNode.type.validContent(Fragment.from(slice.content))) {
      ...
      tr.setSelection(TextSelection.near(tr.doc.resolve(insertPosition + slice.content.size), -1));
      return;
    }
    ...
    const startSlicePosition = tr.doc.resolve(Math.min(position + slice.size, tr.doc.content.size));
    const newSlicePosition = Math.min(startSlicePosition.pos + slice.content.size - slice.openEnd, tr.doc.content.size);
    const direction = -1;
    tr.setSelection(TextSelection.near(tr.doc.resolve(newSlicePosition), direction));
  } else {
    tr.replaceSelection(slice);
  }
}
...
function flattenList(state, node, nodesArr) {
  ...
  node.content.forEach(child => {
    if (isList(state.schema, child) || child.type === listItem && isList(state.schema, child.firstChild)) {
...
function shouldFlattenList(state, slice) {
  ...
  return node && insideTable(state) && isList(state.schema, node) && slice.openStart > slice.openEnd;
}

function sliceHasAlignmentOrIndentationMarks(slice) {
...
export function handleParagraphBlockMarks(state, slice) {
  ...
  } = state; // If no paragraph in the slice contains alignment or indentation marks, there's no need
  ...
  if (grandparent.type.allowsMarkType(schema.marks.alignment) || grandparent.type.allowsMarkType(schema.marks.indentation)) {
  ...
  return mapSlice(slice, node => {
    if (node.type === schema.nodes.paragraph) {
      return schema.nodes.paragraph.createChecked(undefined, node.content, node.marks.filter(mark => mark.type.name !== 'alignment' && mark.type.name !== 'indentation'));
    }
...
export function handleRichText(slice) {
  return (state, dispatch) => {
    const {
      codeBlock,
      panel
    } = state.schema.nodes; // In case user is pasting inline code,
    ...
    if (allowPredictableLists && (isList(state.schema, slice.content.firstChild) || isList(state.schema, slice.content.lastChild))) {
      insertSlice({
        tr,
        slice
      });
      ...
      if (tr.selection.$from === tr.selection.$to && panelParent && !panelParent.node.textContent) {
        tr = safeInsert(slice.content, tr.selection.$to.pos)(tr); // set selection to end of inserted content
...
export const handleSelectedTable = slice => (state, dispatch) => {
