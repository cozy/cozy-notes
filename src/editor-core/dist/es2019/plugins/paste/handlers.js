...
export function handlePasteAsPlainText(slice, _event) {
  return (state, dispatch, view) => {
    ...
    if (view && view.shiftKey) {
      ...
      const { selection } = tr // <- using the same internal flag that prosemirror-view is using
...
export function handlePastePreservingMarks(slice) {
  return (state, dispatch) => {
    ...
    const isPlainTextSlice =
      ...
      slice.content.firstChild.firstChild.type === text // special case for plainTextSlice & linkMark: merge into existing link
...
function hasInlineCode(state, slice) {
  return (
    slice.content.firstChild &&
    slice.content.firstChild.marks.some(m => m.type === state.schema.marks.code)
  )
}

export function insertSlice({ tr, slice }) {
  const {
    selection,
  ...
  if (panelNode && isEmptyNode(panelNode) && $from.node() === $to.node()) {
    const { from: panelPosition } = selection // if content of slice isn't valid for a panel node, insert slice after
...
export function handleRichText(slice) {
  return (state, dispatch) => {
    const { codeBlock, panel } = state.schema.nodes // In case user is pasting inline code,
