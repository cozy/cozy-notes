...
import { escapeLinks, htmlContainsSingleFile, isPastedFromWord, isPastedFromExcel } from '../util';
...
import { transformSliceNestedExpandToExpand, transformSliceToRemoveOpenExpand } from '../../expand/utils';
...
import { upgradeTextToLists as upgradeTextToListsPredictable, splitParagraphs as splitParagraphsPredictable } from '../../lists-predictable/transforms';
import { md } from '../md';
...
export function createPlugin(schema, cardOptions, sanitizePrivateContent, predictableLists, providerFactory) {
  ...
  return new Plugin({
    ...
    props: {
      handlePaste(view, rawEvent, slice) {
        ...
        const uriList = event.clipboardData.getData('text/uri-list'); // Links copied from iOS Safari share button only have the text/uri-list data type
        ...
        const isRichText = !!html; // Bail if copied content has files
        ...
        if (isPlainText) {
          markdownSlice = getMarkdownSlice(text, slice.openStart, slice.openEnd); // run macro autoconvert prior to other conversions

          if (markdownSlice && handleMacroAutoConvert(text, markdownSlice, cardOptions, extensionAutoConverter)(state, dispatch, view)) {
        ...
        if (isRichText) {
          ...
          slice = linkifyContent(state.schema)(slice); // run macro autoconvert prior to other conversions
