...
export function createLinkInputRule(regexp) {
  ...
  return createInputRule(regexp, (state, match, start, end) => {
    ...
    }])(state.tr.addMark(start - (link.input.length - link.lastIndex), end - (link.input.length - link.lastIndex), markType).insertText(' '));
...
export function createInputRulePlugin(schema) {
  ...
  const urlWithASpaceRule = createLinkInputRule(new LinkMatcher()); // [something](link) should convert to a hyperlink
  ...
  const markdownLinkRule = createInputRule(/(^|[^!])\[(.*?)\]\((\S+)\)$/, (state, match, start, end) => {
    ...
    const tr = state.tr.replaceWith(start + prefix.length, end, schema.text(linkText, [markType]));
