...
export const getToolbarConfig = (allowCopyToClipboard = false) => (state, {
  ...
  if (state.doc.nodeAt(pos)) {
    ...
    const language = node && node.node.attrs ? node.node.attrs.language : undefined;
    const options = [{
      label: '(None)',
      value: '',
      alias: ['none']
    }].concat(languageList.map(lang => ({
      label: lang.name,
    ...
    const languageSelect = {
      ...
      defaultValue: language ? options.find(option => option.value === language) : undefined,
