    ...
    handleClick(view, clickPos, event) {
      ...
      const clickWasNearACodeMark =
        ...
        (($click.nodeBefore && code.isInSet($click.nodeBefore.marks)) ||
          ($click.nodeAfter && code.isInSet($click.nodeAfter.marks))) // Find the starting position of the clicked dom-element
      ...
      if (
        ...
        clickedDOMElementPosition && // if click did not occur at a text node or end of paragraph, then
        ...
        const nodeNextToClick =
          $click.nodeBefore && code.isInSet($click.nodeBefore.marks)
            ? $click.nodeAfter
            : $click.nodeBefore // Need to set the selection here to allow clicking between [code('text'),{<>},emoji()]
