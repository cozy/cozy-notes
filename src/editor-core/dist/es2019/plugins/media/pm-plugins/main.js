...
const createDropPlaceholder = allowDropLine => {
  ...
  if (allowDropLine) {
    ReactDOM.render( /*#__PURE__*/React.createElement(DropPlaceholder, {
      type: 'single'
    }), dropPlaceholder);
  } else {
    ReactDOM.render( /*#__PURE__*/React.createElement(DropPlaceholder), dropPlaceholder);
  }
  ...
  constructor(_state, options, reactContext, mediaOptions, _dispatch) {
    ...
    _defineProperty(this, "setMediaProvider", async mediaProvider => {
      ...
      if (this.allowsUploads) {
        ...
        if (this.mediaProvider.uploadParams && this.uploadMediaClientConfig) {
          await this.initPickers(this.mediaProvider.uploadParams, PickerFacade, this.reactContext);
        } else {
          this.destroyPickers();
        }
      } else {
    ...
    _defineProperty(this, "insertFile", (mediaState, onMediaStateChanged, pickerType) => {
      ...
      if (!isEndState(mediaStateWithContext)) {
        ...
        this.addPendingTask(new Promise(resolve => {
    ...
    _defineProperty(this, "shouldUseMediaPickerPopup", () => {
      if (!this.mediaOptions || this.mediaOptions && this.mediaOptions.useMediaPickerPopup === undefined) {
    ...
    _defineProperty(this, "selectedMediaContainerNode", () => {
      ...
      if (selection instanceof NodeSelection && (selection.node.type === schema.nodes.mediaSingle || selection.node.type === schema.nodes.mediaGroup)) {
  ...
  async initPickers(uploadParams, Picker, reactContext) {
    ...
    if (!pickers.length) {
      ...
      if (this.options.customMediaPicker) {
        ...
        const popupPicker = new Picker('popup', pickerFacadeConfig, defaultPickerConfig).init();
...
export const createPlugin = (_schema, options, reactContext, dispatch, mediaOptions) => {
  const dropPlaceholder = createDropPlaceholder(mediaOptions && mediaOptions.allowDropzoneDropLine);
  return new Plugin({
    state: {
      init(_config, state) {
