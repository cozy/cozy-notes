...
function mapping(tr, pluginState) {
  if (pluginState && pluginState.mediaPos !== null) {
    return { ...pluginState, mediaPos: tr.mapping.map(pluginState.mediaPos) }
  }
...
function onSelectionChanged(tr) {
  ...
  if (mark) {
    return { ...initialState, mediaPos, editable: true, link: mark.attrs.href }
  }

  return { ...initialState, mediaPos }
}
