...
export const showLinkingToolbarWithMediaTypeCheck = (
  editorState,
  dispatch,
  editorView
) => {
  if (dispatch && editorView) {
    ...
    checkMediaType(mediaNode, mediaClientConfig).then(mediaType => {
      if (
        (mediaType === 'external' || mediaType === 'image') && // We make sure the selection and the node hasn't changed.
...
function toggleLinkMark(tr, state, { forceRemove = false, url }) {
  const mediaLinkingState = getMediaLinkingState(state)
  ...
  const toggleBlockLinkMark = createToggleBlockMarkOnRange(
    linkMark,
    (prevAttrs, node) => {
...
const fireAnalyticForMediaLink = (
  tr,
  state,
  action,
...
export const setUrlToMedia = (url, inputMethod) =>
  createMediaLinkingCommand(
    {
