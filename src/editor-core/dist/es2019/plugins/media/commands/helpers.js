...
export const findMediaSingleNode = (mediaPluginState, id) => {
  const { mediaNodes } = mediaPluginState // Array#find... no IE support
...
export const findAllMediaSingleNodes = (mediaPluginState, id) => {
  ...
  return mediaNodes.filter(
    nodeWithHandler => nodeWithHandler.node.attrs.id === id
  )
}
...
export const isMobileUploadCompleted = (mediaPluginState, mediaId) =>
  !!mediaPluginState.mediaOptions && // This flag tells us that it's a 'mobile' env.
  ...
  typeof mediaPluginState.mobileUploadComplete[mediaId] === 'boolean'
    ...
    : undefined
export const updateAllMediaNodesAttrs = (id, attrs, isMediaSingle) => (
  state,
  dispatch
) => {
...
export const updateMediaNodeAttrs = (id, attrs, isMediaSingle) => (
  state,
  dispatch
) => {
  const mediaPluginState = mediaPluginKey.getState(state)
