...
import { createInputRule as defaultCreateInputRule, defaultInputRuleHandler, leafNodeReplacementCharacter, instrumentedInputRule } from '../../../utils/input-rules';
...
export const insertList = (state, listType, start, end) => {
  ...
  let tr = state.tr.delete(start, end).split(start); // If node has more content split at the end of autoformatting.
...
function getBulletListInputRules(schema) {
  ...
  const asteriskRule = defaultInputRuleHandler(createInputRule(/^\s*([\*\-]) $/, schema.nodes.bulletList), true);
  const leafNodeAsteriskRule = defaultCreateInputRule(new RegExp(`${leafNodeReplacementCharacter}\\s*([\\*\\-]) $`), (state, _match, start, end) => {
...
function getOrderedListInputRules(schema) {
  ...
  const numberOneRule = defaultInputRuleHandler(createInputRule(/^(1)[\.\)] $/, schema.nodes.orderedList), true);
  const leafNodeNumberOneRule = defaultCreateInputRule(new RegExp(`${leafNodeReplacementCharacter}(1)[\\.\\)] $`), (state, _match, start, end) => {
