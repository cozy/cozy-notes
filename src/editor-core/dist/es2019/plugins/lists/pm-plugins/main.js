...
export const createPlugin = dispatch => new Plugin({
  state: {
    init: () => ({
    ...
    apply(tr, pluginState, _, state) {
      ...
      const bulletListDisabled = !(bulletListActive || orderedListActive || isWrappingPossible(bulletList, state));
      const orderedListDisabled = !(bulletListActive || orderedListActive || isWrappingPossible(orderedList, state));
  ...
  view: () => {
    return {
      update: ({
        ...
        const listParent = findParentNodeOfType([bulletList, orderedList])(state.tr.selection);
