...
const extractListFromParagaph = (node, parent, schema) => {
  const {
    hardBreak,
    bulletList,
    orderedList
  ...
  const listTypes = [bulletList, orderedList]; // wrap each line into a listItem and a containing list

  const listified = content.map((child, index) => {
    const listMatch = getListType(child, schema);
    const prevChild = index > 0 && content[index - 1]; // only extract list when preceded by a hardbreak
    ...
    const [nodeType, length] = listMatch; // convert to list item
    ...
    const newList = nodeType.createChecked(undefined, [listItemNode]); // Check whether our new list is valid content in our current structure,
  ...
  const fragment = lastTr ? lastTr.doc.content : Fragment.from(listified); // try to re-wrap fragment in paragraph (which is the original node we unwrapped)
