...
export const calcJoinListScenario = (walkNode, $head, tr) => {
  ...
  const headInList = isPosInsideList($head); //Must be at the start of the selection of the first child in the listItem
  ...
  const prevParentLastChildIsList = $prev.parent.lastChild && ($prev.parent.lastChild.type.name === 'orderedList' || $prev.parent.lastChild.type.name === 'bulletList');
  const prevParentLastChildIsParagraph = $prev.parent.lastChild && $prev.parent.lastChild.type.name === 'paragraph'; // Will search for the possible last node for case 4 (where the list could be indented multiple times)
