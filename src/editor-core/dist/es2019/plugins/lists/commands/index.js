...
export const backspaceKeyCommand = (state, dispatch) => {
  return baseCommand.chainCommands(
    listBackspace, // if we're at the start of a list item, we need to either backspace
    // directly to an empty list item above, or outdent this node
    filter(
      [
        isEmptySelectionAtStart, // list items might have multiple paragraphs; only do this at the first one
        isFirstChildOfParent,
        canOutdent
      ],
...
export function toggleListCommand(listType) {
  return function(state, dispatch, view) {
    if (dispatch) {
      dispatch(
        state.tr.setSelection(adjustSelectionInList(state.doc, state.selection))
      )
    }
...
const schemaTypeToAnalyticsType = type =>
  type === 'bulletList'
    ? ACTION_SUBJECT_ID.FORMAT_LIST_BULLET
    : ACTION_SUBJECT_ID.FORMAT_LIST_NUMBER // TODO: Toggle list command dispatch more than one time, so commandWithAnalytics doesn't work as expected.
