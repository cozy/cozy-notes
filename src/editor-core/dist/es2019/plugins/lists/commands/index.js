...
import { findParentNodeOfTypeClosestToPos, findPositionOfNodeBefore, hasParentNodeOfType, findParentNodeClosestToPos } from 'prosemirror-utils';
...
import { ACTION, ACTION_SUBJECT, ACTION_SUBJECT_ID, addAnalytics, EVENT_TYPE, INDENT_DIRECTION, INDENT_TYPE, INPUT_METHOD, withAnalytics } from '../../analytics';
import { GapCursorSelection } from '../../selection/gap-cursor-selection';
import { liftFollowingList, liftSelectionList } from '../transforms';
import { canJoinToPreviousListItem, canOutdent, isInsideListItem } from '../utils';
import { listBackspace } from './listBackspace';
import { listDelete } from './listDelete';
const maxIndentation = 5;
export const deletePreviousEmptyListItem = (state, dispatch) => {
  const {
    $from
  } = state.selection;
  const {
...
export const joinToPreviousListItem = (state, dispatch) => {
  const {
    $from
  } = state.selection;
  const {
...
export const backspaceKeyCommand = (state, dispatch) => {
  ...
  isFirstChildOfParent, canOutdent], baseCommand.chainCommands(deletePreviousEmptyListItem, outdentList(INPUT_METHOD.KEYBOARD))), // if we're just inside a paragraph node (or gapcursor is shown) and backspace, then try to join
  // the text to the previous list item, if one exists
  filter([isEmptySelectionAtStart, canJoinToPreviousListItem], joinToPreviousListItem))(state, dispatch);
};
export const deleteKeyCommand = listDelete;
/**
...
export function outdentList(inputMethod = INPUT_METHOD.KEYBOARD) {
  return function (state, dispatch) {
    ...
    if (isInsideListItem(state)) {
      ...
      const parentAtStart = findParentNodeOfTypeClosestToPos(state.selection.$from, [state.schema.nodes.bulletList, state.schema.nodes.orderedList]);
      return compose( // 4. Richer indentation analytics to compare against new predictable lists
      withAnalytics({
        action: ACTION.INDENTED,
        actionSubject: ACTION_SUBJECT.LIST,
        actionSubjectId: schemaTypeToAnalyticsType( // ! - It's guaranteed that if we've gotten to this point that there is a listType parent. We just don't know which one.
  ...
  return function (state, dispatch) {
    ...
    if (isInsideListItem(state)) {
      ...
      if (sinkable) {
        ...
        }), baseListCommand.sinkListItem)(listItem)(state, fakeDispatch);
...
export const toggleList = (state, dispatch, view, listType, inputMethod) => {
  const {
    selection
  } = state;
...
function isInsideList(state, listType) {
  ...
  return parent && parent.type === state.schema.nodes[listType] || grandgrandParent && grandgrandParent.type === state.schema.nodes[listType];
}
...
export function toggleListCommand(listType) {
  return function (state, dispatch, view) {
    ...
    const listNodeType = state.schema.nodes[listType]; // find closest parent of listNodeType from start of selection

    const listParentPos = findParentNodeOfTypeClosestToPos(doc.resolve($from.pos), listNodeType); // determine if end of selection is outside of that list (if selection is in a list at all)

    const isSameListTypeSelected = listParentPos && $to.pos <= listParentPos.pos + listParentPos.node.nodeSize;
...
const getItemAttributes = (state, $pos) => {
  const indentLevel = numberNestedLists($pos, state.schema.nodes) - 1;
  const itemAtPos = findParentNodeOfTypeClosestToPos($pos, state.schema.nodes.listItem); // Get the index of the current item relative to parent (parent is at item depth - 1)
...
const countListItemsInSelection = editorState => {
  const {
    tr,
    schema
  } = editorState;
  ...
  nodePositions.reduce((acc, pos) => {
    const closestParentListNode = findParentNodeClosestToPos(doc.resolve(pos), node => node.type === schema.nodes.listItem);
...
const getCommonListAnalyticsAttributes = state => {
  const {
    selection: {
      $from,
      $to
...
const schemaTypeToAnalyticsType = type => type === 'bulletList' ? ACTION_SUBJECT_ID.FORMAT_LIST_BULLET : ACTION_SUBJECT_ID.FORMAT_LIST_NUMBER; // TODO: Toggle list command dispatch more than one time, so commandWithAnalytics doesn't work as expected.
// This is a helper to fix that.


export const toggleListCommandWithAnalytics = (inputMethod, listType) => {
  return (state, dispatch, view) => {
    if (view && dispatch) {
      ...
      if (parentAtStart) {
        dispatch(addAnalytics(state, view.state.tr, {
          action: ACTION.CONVERTED,
          actionSubject: ACTION_SUBJECT.LIST,
          eventType: EVENT_TYPE.TRACK,
          actionSubjectId: schemaTypeToAnalyticsType(listType),
          attributes: { ...getCommonListAnalyticsAttributes(state),
            transformedFrom: schemaTypeToAnalyticsType(parentAtStart.node.type.name),
            inputMethod
          }
        }));
      }
