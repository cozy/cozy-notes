...
const extractListFromParagraph = (node, parent, schema) => {
  ...
  const fragment = joinedListsTr
    ...
    : Fragment.from(listified) // try to re-wrap fragment in paragraph (which is the original node we unwrapped)
...
const splitIntoParagraphs = ({ fragment, blockMarks = [], schema }) => {
  const paragraphs = []
  let curChildren = []
  let lastNode = null
