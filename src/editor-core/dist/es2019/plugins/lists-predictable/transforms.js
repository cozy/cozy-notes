...
const extractListFromParagraph = (node, parent, schema) => {
  ...
  const fragment = joinedListsTr ? joinedListsTr.doc.content : Fragment.from(listified); // try to re-wrap fragment in paragraph (which is the original node we unwrapped)
