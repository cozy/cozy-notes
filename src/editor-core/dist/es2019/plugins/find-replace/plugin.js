  ...
  if (newMatches.length > 0 && numDecorations === 0) {
    ...
    tr.steps.forEach(step => {
      if (stepHasSlice(step)) {
        ...
        matchesToAdd = [...matchesToAdd, ...newMatches.filter(match => isMatchAffectedByStep(match, step, tr) && !findDecorationFromMatch(decorationSet, match))]; // delete any matches that are missing from the newMatches array and have a
