...
export const activate = () =>
  createCommand(state => {
    ...
    let findText
    let matches
    let index // if user has selected text and hit cmd-f, set that as the keyword
...
export const find = (editorView, containerElement, keyword) =>
  withScrollIntoView(
    createCommand(
      state => {
        const { selection } = state
        ...
        const matches =
          keyword !== undefined
            ...
            : []
        const index = findSearchIndex(selection.from, matches) // we can't just apply all the decorations to highlight the search results at once
...
export const findNext = () =>
  withScrollIntoView(
    createCommand(
      ...
      (tr, state) => {
        const { matches, index } = getPluginState(state) // can't use index from plugin state because if the cursor has moved, it will still be the
...
export const findPrevious = () =>
  withScrollIntoView(
    createCommand(
      ...
      (tr, state) => {
        const { matches } = getPluginState(state) // can't use index from plugin state because if the cursor has moved, it will still be the
...
export const replace = replaceText =>
  withScrollIntoView(
    createCommand(
      state => {
        const pluginState = getPluginState(state)
...
export const replaceAll = replaceText =>
  createCommand(
    {
...
export const addDecorations = decorations =>
  createCommand(state => {
...
export const removeDecorations = decorations =>
  createCommand(state => {
    const { decorationSet } = getPluginState(state)
    return {
      type: FindReplaceActionTypes.UPDATE_DECORATIONS,
...
export const updateFocusElementRef = focusElementRef =>
  createCommand({
