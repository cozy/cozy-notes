...
export const findNext = () => withScrollIntoView(createCommand(state => findInDirection(state, 'next'), (tr, state) => {
  const {
    matches,
    index
  } = getPluginState(state); // can't use index from plugin state because if the cursor has moved, it will still be the
...
const findInDirection = (state, dir) => {
  ...
  if (findText) {
    const searchIndex = findSearchIndex(state.selection.from, matches, dir === 'previous'); // compare index from plugin state and index of first match forward from cursor position
...
export const updateFocusElementRef = focusElementRef => createCommand({
...
const updateSelectedHighlight = (state, nextSelectedIndex) => {
  ...
  const nextSelectedDecoration = findDecorationFromMatch(decorationSet, nextSelectedMatch); // Update decorations so the current selected match becomes a normal match
