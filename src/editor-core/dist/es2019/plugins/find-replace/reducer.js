...
const reducer = getInitialState => (state, action) => {
  switch (action.type) {
    ...
    case FindReplaceActionTypes.UPDATE_DECORATIONS:
      return { ...state, decorationSet: action.decorationSet }
    ...
    case FindReplaceActionTypes.BLUR:
      return { ...state, shouldFocus: false }

    case FindReplaceActionTypes.UPDATE_FOCUS_ELEMENT:
      return { ...state, focusElementRef: action.focusElementRef }

    case FindReplaceActionTypes.TOGGLE_MATCH_CASE:
      return { ...state, shouldMatchCase: !state.shouldMatchCase }
