...
import { ELEMENTS_CHANNEL, isResolvingMentionProvider, SLI_EVENT_TYPE, buildSliPayload } from '@atlaskit/mention/resource';
...
import { createInitialPluginState, pluginKey as typeAheadPluginKey } from '../type-ahead/pm-plugins/main';
  ...
  return {
    ...
    pmPlugins() {
      return [{
        ...
        plugin: ({
          ...
          portalProviderAPI,
          eventDispatcher
        }) => mentionPluginFactory(dispatch, providerFactory, portalProviderAPI, eventDispatcher, fireEvent, options)
      }];
    },
    ...
    pluginsOptions: {
      quickInsert: ({
        ...
        priority: 400,
        keyshortcut: '@',
        icon: () => /*#__PURE__*/React.createElement(IconMention, {
      ...
      typeAhead: {
        ...
        getItems(query, state, _intl, {
          ...
          const pluginState = getMentionPluginState(state);
          const mentions = !prevActive && queryChanged ? EMPTY : pluginState.mentions || EMPTY;
          ...
          const mentionItems = mentions.map(mention => memoizedToItem.call(mention)); // to show invite teammate item only if there is 2 or less mentionable user/team available
          ...
          if (pluginState.mentionProvider && !!query && mentions.length <= 2) {
            ...
            if (shouldTrackInviteItemExposure) {
              ...
              debouncedFireEvent(buildTypeAheadInviteExposurePayload(!!shouldEnableInvite, sessionId, pluginState.contextIdentifierProvider, userRole));
            ...
            if (shouldEnableInvite) {
              ...
              const querySuffix = query.slice(inviteExperimentLastQueryWithResults.length);
        ...
        selectItem(state, item, insert, {
          ...
          const sanitizePrivateContent = options && options.sanitizePrivateContent;
          const mentionInsertDisplayName = options && options.mentionInsertDisplayName;
          const {
            schema
          } = state;
          const pluginState = getMentionPluginState(state);
          ...
          const pickerElapsedTime = typeAheadPluginState.queryStarted ? Date.now() - typeAheadPluginState.queryStarted : 0;
        ...
        switch (name) {
          case 'mentionProvider':
            ...
            providerPromise.then(provider => {
              ...
              provider.subscribe('mentionPlugin', (mentions, query, stats) => {
