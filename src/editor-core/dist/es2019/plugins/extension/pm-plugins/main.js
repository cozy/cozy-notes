...
export const createProviderHandler = view => async (name, provider) => {
...
const createPlugin = (dispatch, providerFactory, extensionHandlers, portalProviderAPI, eventDispatcher, useLongPressSelection = false) => {
  ...
  return new Plugin({
    ...
    view: editorView => {
      ...
      return {
        update: view => {
          ...
          const newElement = getSelectedDomElement(domAtPos, selectedExtension, isContentExtension);
          ...
          if (element !== newElement) {
            ...
            const layout = selectedExtension ? selectedExtension.node.attrs.layout : 'default';
            updateState({
              showContextPanel: false,
              element: newElement,
              showEditButton,
    ...
    props: {
      nodeViews: {
        extension: ExtensionNodeView(portalProviderAPI, eventDispatcher, providerFactory, extensionHandlers),
        bodiedExtension: ExtensionNodeView(portalProviderAPI, eventDispatcher, providerFactory, extensionHandlers),
        inlineExtension: ExtensionNodeView(portalProviderAPI, eventDispatcher, providerFactory, extensionHandlers)
      },
