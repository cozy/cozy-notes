...
export async function onChangeAction(
  ...
  const { processParametersAfter } = getPluginState(editorView.state)
  ...
  const key = Date.now()
  const { positions: previousPositions } = getPluginState(editorView.state)
  await updateState({
    positions: { ...previousPositions, [key]: nodeWithPos.pos }
  ...
  const { positions } = getPluginState(editorView.state)
  ...
  const newNode = buildExtensionNode(
    ...
    { ...node.attrs, parameters: { ...oldParameters, ...newParameters } },
    node.content
  )
