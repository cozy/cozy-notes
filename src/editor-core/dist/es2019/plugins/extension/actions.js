...
import { replaceSelectedNode, replaceParentNodeOfType, isNodeSelection } from 'prosemirror-utils';
...
export const performNodeUpdate = (type, newAttrs, content, shouldScrollIntoView) => (state, dispatch) => {
  ...
  if (!isNodeSelected) {
    ...
    transaction = replaceParentNodeOfType(state.schema.nodes.bodiedExtension, newNode)(transaction); // Replacing selected node doesn't update the selection. `selection.node` still returns the old node
    ...
    selection = NodeSelection.create(transaction.doc, transaction.mapping.map(state.selection.anchor));
  }
...
export const updateExtensionParams = (updateExtension, node, actions) => async (state, dispatch) => {
  const {
    attrs,
    type
  } = node.node;
  ...
  try {
    ...
    if (newParameters) {
      ...
      return performNodeUpdate(type.name, newAttrs, content, true)(state, dispatch);
    }
  } catch {}
};
...
const createUpdateContextActions = ({
  ...
  return {
    editInContextPanel: (transformBefore, transformAfter) => {
      setEditingContextToContextPanel(transformBefore, transformAfter)(state, dispatch);
    },
    editInLegacyMacroBrowser
  };
};
...
export const editSelectedExtension = editorActions => {
  ...
  return editExtension(null, updateExtension)(editorView.state, editorView.dispatch);
};
export const editExtension = (macroProvider, updateExtension) => (state, dispatch) => {
  ...
  if (updateExtension) {
    updateExtension.then(updateMethod => {
      if (updateMethod && dispatch) {
        ...
        updateExtensionParams(updateMethod, nodeWithPos, actions)(state, dispatch);
        return;
      }
