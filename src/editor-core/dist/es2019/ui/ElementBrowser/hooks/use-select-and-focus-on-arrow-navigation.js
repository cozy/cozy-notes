...
const reducer = (state, action) => {
  switch (action.type) {
    ...
    case ACTIONS.FOCUS_SEARCH:
      return { ...state, focusedItemIndex: undefined, focusOnSearch: true }
...
const moveReducer = (state, action) => {
  ...
  if (action.payload.step && state.selectedItemIndex < action.payload.step) {
    return { ...state, focusOnSearch: true, focusedItemIndex: undefined }
  }
  ...
  return { ...state, focusedItemIndex: safeIndex, selectedItemIndex: safeIndex }
}
...
function useSelectAndFocusOnArrowNavigation(listSize, step) {
  ...
  const reset = useCallback(listSize => {
    let payload = { ...initialState, listSize }
    dispatch({
