...
const clickAreaClickHandler = (view, event) => {
  ...
  const isClickOutsideEditor = (targetIsNotContentArea || tragetIsNotChildOfContentArea || editorIsNotInFocus) && !isInputClicked && !isTextAreaClicked && !isPopupClicked;
...
const outsideProsemirrorEditorClickHandler = (view, event) => {
  ...
  const isGapCursorSet = setCursorForTopLevelBlocks(event, dom, view.posAtCoords.bind(view), editorFocused)(view.state, dispatch);
